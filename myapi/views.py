  ...
  def get(self, request, id=None, format=None):
    ...
    if limit is not None:
      ...
      if offset is not None:
        offset = int(offset)
        stu = Blog.objects.all()[offset:offset+limit]  
      else:
        stu = Blog.objects.all()[:limit]
    else:
      stu = Blog.objects.all()
    serializer = BlogSerializer(stu, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK, headers={'Accept':'application/json; indent=4'})
...
class AuthBlogAPI(BlogAPI):
  permission_classes = [IsAuthenticated]


class AuthCommentAPI(CommentAPI):
  permission_classes = [IsAuthenticated]
